...
//        return tilesOutOfPlace(this, goal);
    ...
    public int getDepth(State goal) {
//        return tilesOutOfPlace(this, goal);
        return tilesOutOfPlace(this, goal);
    }
    ...
    public static int tilesOutOfPlace(State current, State goal) {
        ...
        for(int r = 0; r < c.tiles.length; r++){
            for(int col = 0; col < c.tiles[0].length; col++){
                if(c.tiles[r][col] == 0) {
                    continue;
                }
                if(c.tiles[r][col] != g.tiles[r][col]){
                    count++;
                }
            }
        }
	...
	// compute the sum
        PuzzleState c = (PuzzleState)current;
        PuzzleState g = (PuzzleState)goal;
        int size = c.getTiles().length * c.getTiles().length;
        for (int i = 1; i < size; i++) {
            loc = c.getLocation(i);
            des = g.getLocation(i);
            sum += Math.abs(loc.column - des.column)+Math.abs(loc.row - des.row);
        }
        return sum;
    }
